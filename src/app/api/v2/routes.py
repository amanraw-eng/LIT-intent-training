"""v2 routes: 15-intent model (model2 architecture) - same request/response
contract as v1 (api/v1/routes.py), served from a separate model/service so
v1 keeps running unaffected. Schemas are reused as-is from v1 since the
response shapes are identical; only the model behind them differs.
"""
...
@router.get("/intent/v2/health", response_model=HealthResponse)
async def health():
    if service_v2.model is None:
        raise HTTPException(
            status_code=503,
            detail={
                "status": "not_ready",
                "device": service_v2.device,
                "model_type": MODEL_TYPE,
                "num_classes": service_v2.num_classes,
                "model_output_classes": service_v2.model_output_classes,
            },
        )
...
def _build_inference_error_detail(exc: Exception, file: UploadFile, payload: bytes, k_value: int) -> dict:
    logger.exception("=" * 56)
    logger.exception("INFERENCE REQUEST FAILED (v2)")
    logger.exception("File: %s", file.filename)
    logger.exception("File size: %d bytes", len(payload))
    logger.exception("Model classes from config: %d", service_v2.num_classes)
    logger.exception("Model output classes: %s", service_v2.model_output_classes)
    logger.exception("top_k: %d", k_value)
    logger.exception("Device: %s", service_v2.device)
...
@router.post(
    "/intent/v2/audio/transcriptions",
...
async def create_transcription(
    ...
    if service_v2.model is None:
        logger.error("Inference request received while v2 model is not ready")
        raise HTTPException(
            status_code=503,
            detail={
    ...
    try:
        ranked_intents, duration = await service_v2.predict(payload, suffix, k_value)
    ...
    logger.info("REQUEST SUCCESS (v2) | intent=%s | confidence=%.6f", top_intent, top_confidence)
